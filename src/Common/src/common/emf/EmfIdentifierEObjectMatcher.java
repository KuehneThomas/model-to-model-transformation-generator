...
public class EmfIdentifierEObjectMatcher implements IEObjectMatcher {
	/**
	 * This instance might have a delegate matcher. The delegate matcher will be called when no ID is found and its results are aggregated with the current matcher.
	 */
	private Optional<IEObjectMatcher> delegate;

	/**
	 * This will be used to determine what represents the "identifier" of an EObject. By default, we will use the following logic, in order (i.e. if condition 1 is fulfilled, we will not try conditions 2 and 3) :
	 * <ol>
	 ...
	 */
	public EmfIdentifierEObjectMatcher() {
	 ...
	 */
	public EmfIdentifierEObjectMatcher(IEObjectMatcher delegateWhenNoID) {
	 ...
	 */
	public EmfIdentifierEObjectMatcher(Function<EObject, String> idComputation) {
	 ...
	 *            the function used to compute the ID.
	 */
	public EmfIdentifierEObjectMatcher(IEObjectMatcher delegateWhenNoID, Function<EObject, String> idComputation) {
	...
	protected Set<Match> matchPerId(Iterator<? extends EObject> leftEObjects,
		...
		// Adaption: since noId objects are compared using the fallback - add the not matched too:
		idToMatch.values().stream()
				.filter(m -> m.getLeft() == null || m.getRight() == null)
				.forEach(m -> {
					if (m.getLeft() != null) {
						leftEObjectsNoID.add(m.getLeft());

						m.setOrigin(null);

						final EObject parentEObject = m.getLeft().eContainer();
						final Match parent = leftEObjectsToMatch.get(parentEObject);
						if (parent != null) {
							parent.getSubmatches().remove(m);
						}
					}
					if (m.getRight() != null) {
						rightEObjectsNoID.add(m.getRight());

						m.setOrigin(null);

						final EObject parentEObject = m.getRight().eContainer();
						final Match parent = rightEObjectsToMatch.get(parentEObject);
						if (parent != null) {
							parent.getSubmatches().remove(m);
						}
					...
					if (m.getOrigin() != null) {
						originEObjectsNoID.add(m.getOrigin());

						final EObject parentEObject = m.getOrigin().eContainer();
						final Match parent = originEObjectsToMatch.get(parentEObject);
						if (parent != null) {
							parent.getSubmatches().remove(m);
						}			
					...
					matches.remove(m);
					m.getSubmatches().forEach(sm -> matches.add(sm));
	...
	public static class DefaultIDFunction implements Function<EObject, String> {
		 ...
		 * @return The identifier for that EObject if we could determine one. <code>null</code> if no condition (see description above) is fulfilled for the given eObject.
		 */
